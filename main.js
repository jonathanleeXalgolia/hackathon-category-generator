...
function detectLanguage(text) {
  ...
  // Retrieve nested values from a record
  function getNestedValue(obj, path) {
    return path.split(".").reduce((current, key) => {
      if (Array.isArray(current)) {
        return current.flatMap((item) => getNestedValue(item, key));
      }
      return current?.[key];
    }, obj);
  }
  ...
  function processCategories(categories, record) {
    ...
    const categoryValues = categories.flatMap((category) => {
      ...
      return category.attributes.flatMap(
        (attr) => getNestedValue(record, attr) || []
      );
    });
